...
{	
	...
	{
		DEFAULT_PROTOTYPE_INITIALIZER_SETTINGS.setParameter(new Parameter("seed", Long.toString(14354354554L)));
		
		DEFAULT_DATASET_SAMPLER_SETTINGS.setParameter(new Parameter("seed", Long.toString(13543455445L)));
		
		DEFAULT_SETTINGS.setParameter(new Parameter("lambdaInsert", Integer.toString(1000)));
		DEFAULT_SETTINGS.setParameter(new Parameter("lambdaPruning", Integer.toString(4000)));
	...
	private final void compute(CompetitiveGraph graph, DatasetSampler sampler)
	{
		...
		while(sampler.hasNext() && graph.numberOfVertices()<maxVertices)
		{
			if(sampler.epoch() != epochCnt)
			{
				...
				System.out.println("size=" + graph.numberOfVertices());
				System.out.println("=====================");
