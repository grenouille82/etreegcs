...
public class MeanSquaredError extends AbstractEvaluationMeasure<FlatClusterModel>
...
{	
	public double compute(FlatClusterModel model) 
	{
		...
		for(int i=0; i<n; i++)
		{
			...
			clusterValues.put(c.getId(), error/c.size());
		}
		
		overallValue = totalError/totalCnt;
		return overallValue;
	}
