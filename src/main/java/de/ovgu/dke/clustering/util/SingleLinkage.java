...
public class SingleLinkage extends AbstractInterClusterDistance<Cluster>
implements InterClusterDistance<Cluster>
{
	...
	public SingleLinkage(ObjectSet dataset, DistanceMeasure metric) 
	{
	...
	public double distance(Cluster a, Cluster b) 
	{
		...
		for(int i : dataA)
		{
			...
			for(int j : dataB)
			{
				...
				d = Math.min(metric.getDistance(x, y), d);
			}
		}
	...
	public double distance(Cluster a, Cluster b, ProximityMatrix matrix) 
	{
		...
		{
			for(int j : dataB)
				d = Math.min(matrix.getValue(i, j), d);
		}
